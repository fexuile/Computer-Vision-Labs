...
def compute_iou(box, boxes, box_area, boxes_area):
    ...
    ## ---------------------------------------------------------- ##
    inter = np.zeros((boxes.shape[0],), dtype=np.float32)
...
class MultiShapeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = np.asarray([random.randint(0, 255) for _ in range(3)])[None, None, :].repeat(self.h, axis=0).repeat(self.w, axis=1)
        img = np.ascontiguousarray(img, dtype=np.uint8)  # Ensure contiguous memory layout for OpenCV
