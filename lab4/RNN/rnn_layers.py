...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # h = h.reshape(N, T, H)
    cache = (h0, Wx, Wh, b, cache)  # Store initial state and weights in cache
...
def rnn_backward(dh, cache):
    ...
    db = np.zeros_like(b)
    dprev_h = np.zeros((N, H))  # Initialize gradient for previous hidden state
    for t in reversed(range(T)):
        step_cache = step_caches[t]
        dnext_h = dh[:, t, :] + dprev_h  # Add gradient from next timestep
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, step_cache)
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dprev_h  # The last dprev_h is the gradient for the initial hidden state
